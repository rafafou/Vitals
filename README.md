...
4. Valide. Le plugin apparaît dans l'onglet **Plugins** de la barre d'outils, groupe
   **Securite**, bouton **Remote Auditor**.
...
1. Clique sur **Remote Auditor** pour ouvrir le panneau (dock à droite).
...
- Détection de flux inter‑modules (suivre un arg passé à `require(...)`).
- Mode « live » : mesurer en playtest le débit réel de chaque remote (spam/flood).
- Export du rapport en Markdown / dans le Output.
- Quick‑fix : insérer automatiquement un squelette de validation au début d'un handler.
